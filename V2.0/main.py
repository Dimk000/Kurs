...
class MyWindow(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.countGraphLabel = QLabel("Количество графиков")
        ...
        self.CS = QLineEdit("0")
        self.quantStepEdit = QLineEdit("0.01")
        ...
        self.countGraphEdit = QLineEdit("1")
        ...
        self.btnDrawGraph.clicked.connect(self.DrawGraph)
        self.btnLoadFromDb.clicked.connect(self.LoadFromDb)
        self.btnCompare.clicked.connect(self.LoadFromDb)
        ...
        self.vboxWin2.addWidget(self.countGraphLabel)
        self.vboxWin2.addWidget(self.countGraphEdit)
    ...
    def SaveToDb(self):
        import datetime
        outfile = open('/home/user/Kurs/V2.0/datas/{:%Y-%m-%d %H-%M-%S}.csv'.format(datetime.datetime.now()), 'w')
    ...
    def LoadFromDb(self):
        import os
        import csv
        quantStep = float(self.quantStepEdit.text())
        ...
        for (dirpath, dirnames, filenames) in os.walk('/home/user/Kurs/V2.0/datas'):
            for filename in filenames:
                ...
                while i < len(temp):
                    ...
                    i += 1
                # MassForRes = np.arange(0.001, 3, 0.001)
                # CSch = 2
                # timeStep = 0.01
                ...
                cmp = SC.solocomp(Mmain, M1)
                Mall.append(cmp)
                # self.drw2(MassForRes, Stemp, WFCoeff1)
                f.close()
    ...
    def DrawGraph(self):
        ...
        ololo.Freq(Xdatag,Ydatag,quantStep)
        pprint.pprint(ololo.Freq(Xdatag,Ydatag,quantStep))
    ...
    def drw2(self, MassForRes, S, WFCoeff):
...
def meh():
    ...
    line, = ax.plot([0, 3, 0.5],[0, 1, 0.5], color = 'green', alpha=0.0)  # empty line
