...
class MyWindow(QtWidgets.QWidget):
    def __init__(self, parent=None):
        from PyQt5.QtWidgets import (QApplication, QFrame, QHBoxLayout, QLabel, QLineEdit, QPlainTextEdit, QPushButton, QRadioButton, QTabWidget,
        ...
        #  поля ввода
        self.StartMasEdit = QLineEdit("0")
        self.StartMasEdit.setFixedWidth(100)

        self.StepMasEdit = QLineEdit("0.1")
        self.StepMasEdit.setFixedWidth(50)

        self.EndMasEdit = QLineEdit("10")
    ...
    def calculate(self):
        ...
        self.draw(Start, Step, End)
        self.database()

    def draw(self, Start, Step, End):
        self.axes.plot(MassForRes, abs(S), label="for {} : {} : {}".format(Start, Step, End))
        # self.axes.plot(MassForRes, abs(S), label=["for", Start, ":", Step, ":", End])
    ...
    def database(self):
        ...
        cur = con.cursor()
        cur.execute('''CREATE TABLE func (x FLOAT, y FLOAT)''')
        for i in range(0, len(MassForRes), 1):
            cur.execute("INSERT INTO func (x, y) VALUES ('%f','%f')"%(MassForRes[i], S[i]))
        con.commit()
        cur.close()
        con.close()
...
#
        ...
        self.canvas.draw()
        self.plainTextEdit.setPlainText("Коэффициент передачи = {} \n\nω - относительная частота = {}".format(round(abs(S[count]), 10), MassForRes[count]))
